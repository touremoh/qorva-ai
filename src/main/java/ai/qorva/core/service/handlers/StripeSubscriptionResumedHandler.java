...
public class StripeSubscriptionResumedHandler implements StripeEventHandler {
	...
	public java.util.Set<String> eventTypes() {
		return java.util.Set.of("customer.subscription.resumed");
	}
	...
	@Autowired
	public StripeSubscriptionResumedHandler(TenantService tenantService, StripeEventLogRepository repository, StripeEventMapper evtMapper) {
	...
	public void handle(StripeObject obj) throws QorvaException {
		log.info("Handling customer.subscription.resumed event");
		...
		tenant.getSubscriptionInfo().setSubscriptionStatus(SubscriptionStatus.ACTIVE.getValue());
		tenantService.updateOne(tenant.getTenantId(), tenant);

		var eventLog = new StripeEventLogDTO();
		eventLog.setEventType("customer.subscription.resumed");
		eventLog.setEventStatus(SubscriptionStatus.ACTIVE.getValue());
		...
		log.info("Subscription resumed for customer {}. Status restored to ACTIVE.", stripeCustomerId);
	}
}
