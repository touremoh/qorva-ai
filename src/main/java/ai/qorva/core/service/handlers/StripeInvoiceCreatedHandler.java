...
@Slf4j
@Service
public class StripeInvoiceCreatedHandler extends AbstractStripeLoggingHandler {

	@Autowired
	public StripeInvoiceCreatedHandler(StripeEventLogRepository repository, StripeEventMapper evtMapper, TenantService tenantService, UserRepository userRepository) {
		super(repository, evtMapper, tenantService, userRepository);
	}

	@Override
	public void handle(StripeObject obj) throws QorvaException {
		log.info("Handling invoice.created event");
		...
		String subscriptionId = invoice.getParent() != null && invoice.getParent().getSubscriptionDetails() != null
			...
			: null;
		saveEventLog("invoice.created", invoice.getCustomer(), subscriptionId, invoice.getStatus());
	}
}
