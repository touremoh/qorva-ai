...
@Slf4j
@Service
public class StripeInvoicePaidHandler extends AbstractStripeLoggingHandler {

	@Autowired
	public StripeInvoicePaidHandler(StripeEventLogRepository repository, StripeEventMapper evtMapper, TenantService tenantService, UserRepository userRepository) {
	...
	public void handle(StripeObject obj) throws QorvaException {
		...
		String subscriptionId = invoice.getParent() != null && invoice.getParent().getSubscriptionDetails() != null
			...
			: null;
		saveEventLog("invoice.paid", invoice.getCustomer(), subscriptionId, invoice.getStatus());
	}
}
