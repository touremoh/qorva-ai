...
- `experienceRequirements`: derive `minYearsOfExperience`, `minRelevantYears` (≤ minYearsOfExperience) and `seniorityLevel` (one of `junior`, `mid`, `senior`, `lead`) from the description.
- `locationPreferences`: extract locations mentioned in the description into `allowedLocations` (empty list if none); `remoteAllowed` true if remote/hybrid work is possible; `strictness` one of `low`, `medium`, `high` (high only when the description makes location a hard requirement).
- `industryPreferences`: relevant industries in `preferredIndustries` (empty list if unclear); `strictness` one of `low`, `medium`, `high`.
